...
`
	...
	SQLCreateIndexCalibration = `
	CREATE UNIQUE INDEX ix_calibration 
	ON calibration (elr, total_yards_from, total_yards_to)
	`

	SQLInsertCalibration = `
	INSERT INTO calibration(
		elr, 
		total_yards_from, 
		total_yards_to, 
		linear_offset_from_m, 
		linear_offset_to_m, 
		linear_offset_from_norm, 
		linear_offset_to_norm, 
		accuracy, 
		quarter_mile_norm_y
	) values(?,?,?,?,?,?,?,?,?)
	`
	...
	SQLCreateIndexStatistics = `
	CREATE UNIQUE INDEX ix_statistics_elr 
	ON statistics (elr)
	`
	...
	INSERT INTO statistics(
		...
		seg_len_median, 
		seg_len_std, 
		quarter_mile_norm_count, 
		quarter_mile_norm_min, 
		quarter_mile_norm_max, 
		quarter_mile_norm_mean, 
		quarter_mile_norm_median, 
		quarter_mile_norm_std
	) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
	`

	QryAllELRs = `
	SELECT elr, total_yards_from, total_yards_to, shape_length_m, geometry 
	FROM cl
	`

	QryAllMPsInELR = `
	SELECT total_yards_from, geometry 
	FROM mp 
	WHERE elr=? 
